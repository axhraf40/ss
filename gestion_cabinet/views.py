...
def dashboard(request):
    ...
    if hasattr(user, 'patient_profile'):
        ...
        # Ajouter les 5 derniers patients
        context['patients_recents'] = Patient.objects.all().order_by('-user__date_joined')[:5]
    ...
    if request.method == 'POST':
        ...
        if patient_id:
            try:
                ...
                pass
        form = RendezVousPatientForm(initial=initial, user=request.user)
...
def definir_montant_rdv(request, rdv_id):
    ...
    # Vérifier si le rendez-vous n'est pas annulé
    if rendez_vous.statut == 'ANNULE':
        messages.error(request, "Impossible de définir un montant pour un rendez-vous annulé.")
        return redirect('rendez_vous_list')
...
def create_pending_appointment(request):
    """Crée un rendez-vous à partir des données stockées dans la session."""
    if 'rendez_vous_data' in request.session:
        try:
            data = request.session['rendez_vous_data']
            rendez_vous = RendezVous()
            rendez_vous.dentiste_id = data['dentiste']
            rendez_vous.date_heure = datetime.fromisoformat(data['date_heure'])
            rendez_vous.motif = data['motif']
            rendez_vous.patient = request.user.patient_profile
            rendez_vous.statut_paiement = 'EN_ATTENTE'
            rendez_vous.montant = 0
            rendez_vous.save()
            messages.success(request, 'Votre rendez-vous a été créé avec succès !')
            del request.session['rendez_vous_data']
            return True
        except Exception as e:
...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                # Vérifier s'il y a un rendez-vous en attente dans la session
                if create_pending_appointment(request):
                    return redirect('rendez_vous_list')
    ...
    return render(request, 'gestion_cabinet/login.html', {'form': form})
...
def patient_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # S'assurer que l'utilisateur a les permissions de base
            user.is_active = True
            user.save()
        ...
        form = UserRegistrationForm(initial={'role': 'PATIENT'})
    return render(request, 'gestion_cabinet/patient_form.html', {
        'form': form,
        'is_update': False
    })
...
def patient_update(request, patient_id):
    patient = get_object_or_404(Patient, id=patient_id)
    if request.method == 'POST':
        form = UserRegistrationForm(request.POST, instance=patient.user)
        if form.is_valid():
            user = form.save(commit=False)
            user.role = 'PATIENT'  # S'assurer que le rôle reste patient
            user.save()
            messages.success(request, 'Compte patient modifié avec succès!')
            return redirect('patient_list')
    else:
        form = UserRegistrationForm(instance=patient.user)
    return render(request, 'gestion_cabinet/patient_form.html', {
        'form': form,
        'patient': patient,
        'is_update': True
...
def patient_delete(request, patient_id):
    ...
    if request.method == 'POST':
        user = patient.user
        user.delete()  # Cela supprimera aussi le patient à cause de la relation OneToOne
        messages.success(request, 'Compte patient supprimé avec succès!')
        return redirect('patient_list')
