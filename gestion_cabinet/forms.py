...
class RendezVousPatientForm(forms.ModelForm):
    ...
    class Meta:
        model = RendezVous
        fields = ['patient', 'dentiste', 'date_heure', 'motif']
        widgets = {
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        # Si l'utilisateur est un patient, masquer le champ patient et le remplir automatiquement
        if user and hasattr(user, 'patient_profile'):
            self.fields['patient'].widget = forms.HiddenInput()
            self.fields['patient'].initial = user.patient_profile
            self.fields['patient'].required = False
        else:
            # Pour les secrétaires et dentistes, afficher la liste des patients
            self.fields['patient'].queryset = Patient.objects.all().order_by('user__last_name', 'user__first_name')
